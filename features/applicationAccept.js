...
module.exports = {
    ...
    async applicationAccept(interaction, mongoClient, client) {
        ...
        let acceptedUserDB = (await readData(mongoClient, { "userID": acceptedUser.id }));

        //  checks if the accepted user is in database
        if (acceptedUserDB.length === 0) {

            interaction.editReply({
                content: "You cannot continue this process, due to timeout!",
                ephemeral: true
            });
        ...
        acceptedUserDB = acceptedUserDB[0];

        //  removing this current team's ID from applied user's "appliedTeams" array
        if (acceptedUserDB.appliedTeams.includes(teamID)) {

            let arr = acceptedUserDB.appliedTeams;
            let index = arr.indexOf(teamID);
            arr.splice(index, 1);
        ...
        if (acceptedUserDB.teamName !== null) {
